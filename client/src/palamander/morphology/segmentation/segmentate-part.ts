...
import { SegmentsSpec, addCurve, createNoodleLimb , createRotation, createDefault} from "./segments";
...
const segmentateClaw: SegmentationFunc = (parent: Segment, section: Section): Segment[] => {
  ...
  const upperArmWaveSpec = { range: 30, period: preset.period.deliberate, offset: section.offset, acceleration: 0 };
...
const segmentateCurl: SegmentationFunc = (parent: Segment, section: Section): Segment[] => {
  ...
  const waveSpec = { range: 120 / spec.count, period: preset.period.deliberate, offset: section.offset };
...
const segmentateFishTail: SegmentationFunc = (parent: Segment, section: Section): Segment[] => {
  ...
  const waveSpec = { range: 30/count, period: preset.period.deliberate, offset: section.offset };
...
const segmentateHair: SegmentationFunc = (parent: Segment, section: Section): Segment[] => {
  ...
  const generateGentleCurl = (i: number) => {
    return [
      createRotationSpec({ range: 5, period: preset.period.relaxed, offset: section.offset }),
      createCurlSpec({ range: 30, period: preset.period.deliberate, offset: section.offset }, i)
    ];
  };
  const segments = createDefault(parent, spec, generateGentleCurl);
  return segments;
...
const segmentateMandible: SegmentationFunc = (parent: Segment, section: Section): Segment[] => {
  ...
  const waveSpec = { range: 5, period: preset.period.deliberate, offset: section.offset, acceleration: 0 };
...
const segmentateMonkeyArm: SegmentationFunc = (parent: Segment, section: Section): Segment[] => {
  ...
  const upperArmWaveSpec = { range: 45, period: preset.period.deliberate, offset: section.offset };
  ...
  const forearmWaveSpec = { range: 20, period: preset.period.deliberate, offset: section.offset + Math.PI };
...
const segmentateNoodleLimb: SegmentationFunc = (parent: Segment, section: Section): Segment[] => {
  ...
  const noodleWave = { range: 10, period: preset.period.relaxed, offset: section.offset };
...
const segmentateRigidLeg: SegmentationFunc = (parent: Segment, section: Section): Segment[] => {
  ...
  const waveSpec = { range: 30, period: preset.period.deliberate, offset: section.offset };
...
const segmentateSimpleLimb: SegmentationFunc = (parent: Segment, section: Section): Segment[] => {
  ...
  const waveSpec = { range: 10, period: preset.period.relaxed, offset: section.offset };
  const segments = addCurve(createRotation(parent, spec, waveSpec), curve);
  return segments;
}
