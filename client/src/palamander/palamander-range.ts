...
function wrapToRange(coordinate: number, range: number): number {
  if (coordinate < range)
    coordinate += Math.ceil((range - coordinate) / range) * range;
...
function calculateBottomRightEdges(
  circle: Circle,
  range: Range,
  spawn: Coords,
  ...
  const wrappedCenters = [-1, 1]
    .map((xDir) =>
      [-1, 1].map((yDir) => {
        const edgeOffset = {
          x: pixelCircle.radius * xDir,
          y: pixelCircle.radius * yDir,
        };
  ...
  const centerMap = wrappedCenters.reduce(
    (acc: { [key: string]: Coords }, center) => {
      acc[JSON.stringify(center)] = center;
      return acc;
    },
    {},
  );
  const uniquewrappedCenters = Object.entries(centerMap).map(
    ([_, edge]) => edge,
  );

  // Return bottom right edge
  const lowerRightOffset = stretch(
    { x: pixelCircle.radius, y: pixelCircle.radius },
    -1,
  );
...
class DisplayRange {
  ...
  setRange(rect: DOMRect) {
    ...
    this.#spawn = shift(
      this.#range.upperLeft,
      stretchByElement(this.#range.range, this.#spawnFactor),
    );
    return this;
  }
  ...
  styleSegment(
    ...
    const bottomRightEdges = calculateBottomRightEdges(
      circle,
      this.#range,
      this.#spawn,
      magnification,
...
export {
  DisplayRange,
  generateWindowDisplayRange,
  generateBoundedDisplayRange,
};
