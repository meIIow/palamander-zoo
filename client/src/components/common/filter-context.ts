...
import { ColorFilter, initColorFilter, PalamanderFilters } from './color-filter.ts';
...
type PalFiltersContextValue = { filters: PalamanderFilters, set: (type: string, color: string) => void }
const palFiltersContextDefault = { filters: {}, set: (_: string, __: string) => {} }
const PalFiltersContext = createContext<PalFiltersContextValue>(palFiltersContextDefault);

function reduceColorFilter(filters: ColorFilter, action: ColorFilterAction): ColorFilter {
  switch (action.type) {
    case 'CLEAR':
